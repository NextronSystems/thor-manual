...
1. An :ref:`other/debugging:antivirus or edr suspends thor` (>98%)
2. A "paused" command line window due to :ref:`other/debugging:windows quick edit mode` (<1%)
3. A :ref:`other/debugging:constant high system load` that causes THOR to stay back and wait for an idling CPU (<1%)
4. :ref:`other/debugging:the perception of a stalled scan`, which is actually running (<1%)
...
:ref:`other/debugging:Collecting a Diagnostics Pack`. If THOR crashed and
...
The :ref:`other/other-topics:THOR DB` helps us debug situations in which
