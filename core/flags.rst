...
--highlight-timeframe-score <score>
           ...
           .. warning::
              This setting applies only if --highlight-timeframe is specified.
...
--insecure
           ...
           .. warning::
              May expose connections to MITM attacks.
...
--require-elevation
           ...
           .. warning::
              THOR will terminate if it is executed without Administrator (Windows) or root (Unix) privileges.
...
--archive-depth <depth>
           ...
           .. warning::
              Higher values may increase resource usage and scanning time. However, files nested deeper than this limit are ignored.
...
--archive-file-limit <count>
           ...
           Use 0 to disable the limit thus scan all files in an archive.

           In deep mode (--deep), this limit is disabled and all files in an archive are scanned.
...
--signature-include <filter1,filter2,...>
           ...
           Use --list-signatures to list all signatures and their metadata.
...
--signature-exclude <filter1,filter2,...>
           ...
           This can help skip known false positives or irrelevant rules.
...
--deep
           ...
           .. warning::
              This option is not recommended for live scanning productive systems.
...
--delta
           ...
           .. warning::
              Relies on the last scan time stored in the Thor DB. Timestomping may cause false negatives.
...
--lookback <days>
           ...
           .. warning::
              May miss elements if timestamps have been manipulated (e.g. timestomping).
...
--lookback-global
           ...
           This flag is only meaningful if a lookback is active, i.e. if --lookback or --fast is also used.
...
-c, --cpu-limit <percentage>
           ...
           Pause THOR's scanning activity when the system-wide CPU usage exceeds the specified threshold (in percent).
           ...
           Independently of the pausing, and unless --threads is set explicitly, the CPU limit also determines the number of

           parallel threads THOR uses, so that the number of used CPU cores roughly matches the configured percentage.

           Usually, setting --cpu-limit alone (without --threads) is sufficient to control THOR's CPU usage.

           .. warning::
              Very low values combined with CPU usage from other processes can cause THOR to pause permanently, ultimately causing a scan timeout.
...
--no-soft
           ...
           .. warning::
              On low-resource systems, where soft mode would have activated automatically, this may cause system instability.
...
--no-resource-check
           ...
           .. warning::
              May cause system instability if available resources are insufficient. Use with care!
...
--very-low-priority
           ...
           .. warning::
              May significantly extend scan times.
...
--yara-timeout <seconds>
           ...
           .. warning::
              Too low values of this may cause false negatives.
...
--threads <count>
           ...
           If this flag is not set, the number of threads is derived from the CPU limit (see --cpu-limit),

           so that the number of used CPU cores roughly matches the limit.

           Usually, setting --cpu-limit alone is sufficient; use --threads only to control the number of threads explicitly.

           Use 0 to utilize all CPU cores.

           Negative values subtract from the total available cores (e.g., -2 uses all but 2 cores).

           Any explicitly set value, including 0, overrides the thread count that would be derived from the CPU limit.
...
--mft-analysis
           ...
           In deep mode, MFT analysis is enabled by default.
...
--essential-live-registry-only
           ...
           .. warning::
              Decreases scan time, but also significantly reduces the scan coverage within the registry.
...
--collector-no-mft
           ...
           .. warning::
              This may cause some files to be not accessible.
...
--wsl
           ...
           THOR will walk the filesystem of all installed WSL distributions on Windows systems.
...
--c2-memory-analysis
           ...
           .. warning::
              May cause false positives in browsers and legitimate network services.
...
--no-mmap
           ...
           .. warning::
              Disabling memory mapping may increase disk read times.
...
--pure-yara
           ...
           .. warning::
              This option may miss certain threats that are not covered by YARA rules.
...
--process-integrity-full
           ...
           .. warning::
              May cause false positives due to intensified checks.
...
--no-builtin-registry-excludes
           ...
           Disable the built-in live registry key exclude list, allowing THOR to scan live registry keys normally skipped for performance reasons.

           By default, THOR skips a number of less relevant live registry keys to speed up scans.

           These keys are part of the SOFTWARE and SYSTEM hives and are not essential for most scans.


           Keys explicitly excluded via the --exclude-registry-key flag will still be excluded.
...
--no-builtin-path-excludes
           ...
           Disable the built-in path exclude list, allowing THOR to scan paths normally skipped for system-stability reasons.

           By default, THOR excludes a number of paths to avoid interfering with the operating system or other applications.

           This flag is automatically activated when using --deep or --files-all.
...
--no-tui
           ...
           Disable the interactive terminal UI and print log output directly to

           the console instead. The terminal UI is otherwise activated automatically whenever
...
--tui-buffer-size <lines>

           Set the number of log events retained in the scrollable log view

           of the interactive terminal UI. Must be at least 10.
...
--tui-replay-lines <lines>

           Set the number of most recent log events reprinted to the terminal once the

           interactive terminal UI closes. While active, the terminal UI takes over the terminal and

           absorbs all log output that would otherwise be printed to the console; reprinting the most

           recent events on close ensures that short scans do not end without any visible output.

           Use 0 to disable the replay.
...
--audit-trail <file>[=<hostname>_audit_<time>.jsonl.gz]
           ...
           Write an audit trail log file next to the regular scan output.

           The audit trail contains all assessments that THOR made, including

           undetected files and objects, and the relationships between them.

           This is not a scan mode:
             it does not change what THOR scans or reports, it

           only adds a second output file to the current run.

           All restrictions that limit the scan itself (e.g. module filters, time based

           filters, exclusions) therefore also limit the contents of the audit trail,

           while filters that only apply to the output, such as the minimum score, do
...
--remote-password <password>
           ...
           .. warning::
              Passwords are stored in plaintext in the command history.
...
--print-files
           ...
           .. warning::
              This flag can generate a large amount of output.
